 ...
 * @LastEditTime: 2020-07-15 15:55:21
...
* 方法二 
* 思路
* 整体思路和方法一基本一致，都是找因子5的个数，只是方法二是在找因子5的个数时做文章，用耗
* 时更少的方法来找5的个数
* 详解
* 1. n! 这些乘数中，每隔 5 个数，肯定会有一个数至少能拆出一个 5 因子。所以 n / 5 = 至少会出
* 现的 5 的个数。
* 2. 因为 n / 5 并不能完全算出 5 因子的个数，比如若某个数 25 = 5 * 5，分解后得到的 5 也算一
* 个，所以能被 25 因式分解相当于会出现 2 个 5 因子，而第一步中除以 5 算个数的时候已经算 
* 了一个了，所以相当于比之前会多一个 5 因子
* 3. 依此类推，能被 25 5 = 125 因式分解的相当于比之前按 25 因式分解的时候又多出一个 5 因
* 子。能被 125 5 = 625 因式分解的相当于比按 125 因式分解时又多出一个 5 因子。还有 625 * 5 ......
...
/*******
  * 时间复杂度: O(log2 n)
  * 每次递归时，指数 n 减小一半，即 n, n/2, n/4, ....n/2k ，最终趋近于1，令 n/2k = 1 ，
     计算得出计算次数 k = log2n ，所以时间复杂度为 O(log2 n) 。
  * 空间复杂度: O(log2 n)
    每一次递归计算，都需要变量保存底数、下一次计算的幂值以及每次递归计算的结果三个常 量，递归深度为 log2 n ，所以空间复杂度为 log2 n 。
 ***/